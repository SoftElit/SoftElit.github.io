...
/* Демо-версия, бесплатна для использования неограниченным кругом лиц до 30.10.2020 года.
    ...
    */
                ...
                следующий блок IF в обязательном порядке **/
    {
        ...
        else if(c > 390) //свыше 4м. по ширине, 4,5м. по высоте или 30м. по длине - требуется спецпроект, демонтаж ЛЭП и/или дорожных конструкций и привлечение патрулей ДПС.
        {
            ...
            if(c <= 410) //но меньше 4,1м (высота автопоезда в пределах 4,7...4,8м) - на некоторых маршрутом возможно под совпроводом
            {
                cG = 1.5; //надбавка за превышение габаритных размеров для расчётов (за негабаритное разрешение 20т.р. учтены в переменной price)
                price = 20000; //стоимость негабаритного разрешения
            }
        }
    ...
    struct tm dt = {0};  //создание структуры, хранящей дату и время, разбитые на поля
    dt.tm_year = 120;    //задаём год структуры -> 1900 + 120 = 2020
    dt.tm_mon  = 9;      //задаём месяц структуры -> счёт с нуля, т.е. 09 - октябрь
    dt.tm_mday = 30;     //задаём день структуры -> 19 число
    dt.tm_hour = 0;      //задаём час структуры
    dt.tm_min = 0;       //задаём минуту структуры
    dt.tm_sec = 0;       //задаём секунду структуры
    ...
    if(m <= 40000) //в пределах 40 тонн
    {
        ...
        {
            if(c <= (maxc - 60)) //в пределах допустимой по негабаритному разрешению высоты (высота груза = высота автопоезда за вычетом высоты прицепа)
            {
                if(a <= 3000)
                {
